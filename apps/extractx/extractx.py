...
class ExtractXApp:
    ...
    def _row(self, parent, label: str, variable, chooser) -> None:
...
def main() -> None:
    parser = argparse.ArgumentParser(description="ExtractX archive extraction")
    parser.add_argument("--cli", action="store_true", help="run in command-line mode")
    parser.add_argument("archive", nargs="?", help="archive file path")
    parser.add_argument("destination", nargs="?", help="destination directory")
    args = parser.parse_args()

    if args.cli:
        if not args.archive or not args.destination:
            parser.error("--cli requires: archive destination")
        extract_archive(args.archive, args.destination)
        print(f"Extracted: {args.archive} -> {args.destination}")
        return

    if TK_AVAILABLE:
        root = tk.Tk()
        _app = ExtractXApp(root)
        root.mainloop()
        return

    if args.archive and args.destination:
        extract_archive(args.archive, args.destination)
        print(f"Extracted: {args.archive} -> {args.destination}")
        return

    print("Tkinter GUI is unavailable in this Python.")
    print("Use CLI mode: python3 apps/extractx/extractx.py --cli <archive> <destination>")
